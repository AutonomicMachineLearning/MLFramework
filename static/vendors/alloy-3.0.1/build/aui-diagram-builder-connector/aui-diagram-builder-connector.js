...
var Lang = A.Lang,
    ...
    getCubicBezier = function(t, startPos, endPos, cp1, cp2) { //(베지에 곡선)
...
A.PolygonUtil = {
    ...
    drawArrow: function(shape, x1, y1, x2, y2, arrowPoints) {
        ...
        // Slide the arrow position along the line in 5px in polar coordinates (화살표가 shape에 조금 떨어지게 끔 조치)
...
A.Connector = A.Base.create('line', A.Base, [], {
    ...
    draw: function() {
        ...
        var p1 = instance.get('p1'), //출발 지점
            p2 = instance.get('p2'),  // 도착 지점
            c1 = instance.toCoordinate(p1),
            c2 = instance.toCoordinate(p2),
            x1 = c1[0],
            y1 = c1[1],
            x2 = c2[0],
            y2 = c2[1],  //절대좌표를 통해서 좌표 설정
            dx = Math.max(Math.abs(x1 - x2) / 2, 10),
            dy = Math.max(Math.abs(y1 - y2) / 2, 10),
            curveArgs = null,
            nQuadrantSections = 8,
            angle = toDegrees(Math.atan2(y2 - y1, x2 - x1)),
            pseudoQuadrant = Math.round(Math.abs(angle) / (360 / nQuadrantSections)); //각도에 따라서 cp지점을 정함

        if (sign(angle) < 0) { //꺾는 곳
            curveArgs = [
        ...
        shape.clear();
        //ctx.arc(cp[0][0], cp[0][1], 10, 0, 2*Math.PI, false);
        //ctx.arc(cp[1][0], cp[1][1], 10, 0, 2*Math.PI, false);
        shape.moveTo(x1, y1);
