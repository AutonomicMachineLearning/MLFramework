...
var DiagramNode,
    ...
    squareHotPoints=function(x, y){
    var temp = [x, y];
    var points = [[5,5], [5,65], [65,5], [65,65]];
    for(var i=0; i<points.length; i++){
        if(temp===points[i]) return temp;
    }
    if(temp[0]===5 || temp[0]===65) temp[1]=32.5;
    else if(temp[1]===5 || temp[1]===65) temp[0]=32.5;
    return temp;
    },
    findHotPointBestMatch = function(diagramNode1, diagramNode2) {
        ...
        match[0] = squareHotPoints(match[0][0], match[0][1]);
        match[1] = squareHotPoints(match[1][0], match[1][1]);
        console.log(match);
        return match;
    },
